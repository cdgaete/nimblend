...
def test_transposing_to_a_frame_that_is_not_this_one_is_refused():
    with pytest.raises(ValueError, match=r"this array is over \('x', 'y'\)"):
        build().transpose("x")
...
def test_operating_across_different_labels_is_refused():
    ...
    with pytest.raises(ValueError, match="carry different labels"):
        build() + other


def test_a_skip_that_is_not_true_is_refused():
    with pytest.raises(ValueError, match="skip states that the entries present"):
        build().sum(skip=1)


def test_stating_both_skip_and_fill_is_refused():
    with pytest.raises(ValueError, match="state skip= or fill=, not both"):
...
def test_densifying_an_unknown_array_with_a_hole_needs_a_fill():
    ...
    with pytest.raises(ValueError, match="must state fill="):
