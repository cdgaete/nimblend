...
"""
import numpy as np
from typing import List, Dict, Optional, Union, Tuple, Any, Callable, Sequence
...
def from_series(series: Any, dims: Optional[List[str]] = None, value_name: str = "value") -> Array:
    """
    ...
    # Handle polars Series
        ...
        raise TypeError(f"Expected a pandas or polars Series, got {type(series).__name__}")


def _from_pandas_series(series: 'pd.Series', dims: Optional[List[str]], value_name: str) -> Array:
    """Convert a pandas Series to a NimbleNd Array."""
    # Check for MultiIndex
    if not isinstance(series.index, pd.MultiIndex):
        # Handle single-index case by converting to a MultiIndex
        series = series.copy()
        series.index = pd.MultiIndex.from_arrays([series.index], names=[series.index.name or 'dim_0'])
    ...
    # Create the array data filled with NaN
    ...
    position_maps = {
        dim: {val: idx for idx, val in enumerate(unique_coords[dim])}
        for dim in dims
    }
...
def _from_polars_series(series: 'pl.Series', dims: Optional[List[str]], value_name: str) -> Array:
    ...
    if not hasattr(series, '_df'):
        raise ValueError("The polars Series must be a column of a DataFrame to extract dimensional coordinates")
    ...
    for dim_name in dims:
        if dim_name not in df.columns:
            raise ValueError(f"Dimension '{dim_name}' not found in the DataFrame columns")
    ...
    # Create the array data filled with NaN
    shape = tuple(len(vals) for vals in unique_coords.values())
    data = np.full(shape, np.nan, dtype=np.float64)  # Assume float for NaN support
...
def to_series(
    ...
    if format.lower() == 'pandas':
        if not HAS_PANDAS:
            raise ImportError("pandas is required for this functionality. Please install it with pip install pandas")
        return _to_pandas_series(array, dropna, name)

    elif format.lower() == 'polars':
        if not HAS_POLARS:
            raise ImportError("polars is required for this functionality. Please install it with pip install polars")
...
def _to_pandas_series(array: Array, dropna: bool, name: Optional[str]) -> 'pd.Series':
    ...
    indices = np.meshgrid(*[np.arange(len(array.coords[dim])) for dim in array.dims], indexing='ij')
